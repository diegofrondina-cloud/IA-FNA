package com.uncledavecode.fna.dao;
...
@Repository
public interface FnaProductDAO extends JpaRepository<Object, Long> {
    
    String FNA_PRODUCT_QUERY =  "SELECT  " +
            "    fpr.id AS id, " +
            "    fpr.fna_id AS fnaId, " +
            "    fpr.pof_id AS pofId, " +
    ...
    String FNA_PRODUCT_APPEND_RESPONSES_QUERY =
    ...
    @Query(value= FNA_PRODUCT_QUERY +
            FNA_PRODUCT_APPEND_RESPONSES_QUERY +
            "FROM  " +
            "    FNA_PRODUCT_FPR fpr " +
            ...
            "WHERE  " +
            "    fpr.fna_id = :fnaId"+
            "    AND fpr.pof_id = :pofId "+
            "ORDER BY fpr.version ASC ",
            nativeQuery = true)
    List<FnaProductDTO> findAllByFnaId(@Param("fnaId")Long fnaId,@Param("pofId") Long pofId);
    ...
    @Query(value= FNA_PRODUCT_QUERY +
            ...
            "ORDER BY fpr.version ASC ",
            nativeQuery = true)
    List<FnaProductDTO> findAllByFnaIdRestrictedView(@Param("fnaId")Long fnaId,@Param("pofId") Long pofId);
    ...
    @Query(value= FNA_PRODUCT_QUERY +
            ...
            nativeQuery = true)
    FnaProductDTO findByProductId(@Param("fnaProductId")Long fnaProductId);
}